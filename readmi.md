...
- **Вручную через GitHub:** откройте вкладку **Actions** → **Daily Readmi Updates** → **Run workflow**, подтвердите запуск.
- **Локально:**
  1. Убедитесь, что установлены `git` и `openssl`.
  2. Выполните `chmod +x scripts/daily_random_commits.sh` один раз для установки прав.
  3. Запустите `scripts/daily_random_commits.sh`. Скрипт сам создаст три коммита в `readmi.md`.
